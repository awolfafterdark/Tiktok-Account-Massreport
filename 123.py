...
def get_proxies():
    ...
    try:
        ...
        response = requests.get(url, proxies=proxy_log, timeout=60)
...
with open(f"config.json") as f:
    data = json.load(f)
    if data["proxy_scraper"].lower() in ["y", "yes"]:
        check_proxies_file()

def update_console_title():
    global success, failed, generated_agents, total
    success_rate = round(success / total * 100, 2)
    # Removed Windows-specific code to update console title
...
def check_ui():
    ...
    while True:
        ...
        with output_lock:
            ...
            print(f"Reports Sent: {success} | Failed: {failed} | Success Rate: {round(success / total * 100, 2)}%")
            time.sleep(10)
