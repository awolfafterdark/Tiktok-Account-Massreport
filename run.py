...
def update_console_title():
    ...
    print(f'[ Tiktok MassReport ] By H4cK3dR4Du & 452b | Reports Sent : {success} ~ Failed : {failed} ~ Success Rate : {success_rate}%')
    ...
    try:
        ...
        if match_reason:
            ...
            if "Thanks for your feedback" in report.text:
                time_rn = get_time_rn()
                print(f"[ {magenta}{time_rn}{reset} ] | ( {green}+{reset} ) {blue}Reported with successfull to {username} ~ {iduser}")
                success += 1
                total += 1
